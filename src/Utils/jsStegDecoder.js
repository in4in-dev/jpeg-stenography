	...
	constructor.prototype = {
		load: function load(path, returnDCT) {
			...
			path.arrayBuffer()
				...
				.catch((error) => {
					console.error('Ошибка обработки Blob:');
					console.error(error)
				});
		...
		parse: function parse(data) {
			...
			function findSOI(data) {
				for (let i = 0; i < data.length - 1; i++) {
					if (data[i] === 0xFF && data[i + 1] === 0xD8) {
						return i; // Возвращаем индекс начала JPEG
					}
				}
				throw new Error("SOI (0xFFD8) не найден в данных");
			}
